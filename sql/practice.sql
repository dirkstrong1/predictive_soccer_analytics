...
SELECT
    ...
    CASE WHEN home_team_goal = away_team_goal THEN 1 ELSE 0 END AS draws
    FROM Match
    JOIN Team AS t1
    ...
    JOIN league AS l
        USING (country_id)
    WHERE Season = '2008/2009' AND l.name = 'England Premier League';


    CASE WHEN away_team_goal > home_team_goal THEN 1 ELSE 0 END AS a_win,
    CASE WHEN away_team_goal < home_team_goal THEN 1 ELSE 0 END AS a_loss,
    CASE WHEN home_team_goal = away_team_goal THEN 1 ELSE 0 END AS a_draws